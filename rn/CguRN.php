...
class CguRN extends InfraRN {
  ...
  protected function listarDocumentoControlado(WSEntradaListarDocumentoDTO $objWSEntradaListarDocumentoDTO){
    try{
    	...
    	if (!InfraUtil::isBolSinalizadorValido($objWSEntradaListarDocumentoDTO->getStrSinRetornarAndamentoGeracao())) {
    		$objInfraException->adicionarValidacao('Sinalizador de retorno para andamento de geraчуo invсlido.');
    	}
    	 
    	if (!InfraUtil::isBolSinalizadorValido($objWSEntradaListarDocumentoDTO->getStrSinRetornarAssinaturas())) {
    		$objInfraException->adicionarValidacao('Sinalizador de retorno para assinaturas invсlido.');
    	}

    	if (!InfraUtil::isBolSinalizadorValido($objWSEntradaListarDocumentoDTO->getStrSinRetornarPublicacao())) {
    		$objInfraException->adicionarValidacao('Sinalizador de retorno para publicaчуo invсlido.');
    	}

        if (!InfraUtil::isBolSinalizadorValido($objWSEntradaListarDocumentoDTO->getStrSinRetornarDestinatarios())) {
            $objInfraException->adicionarValidacao('Sinalizador de retorno para destinatсrios invсlido.');
        }
        ...
        if ($objDocumentoDTO!='') {
            ...
            if ($objDocumentoDTO->getStrProtocoloDocumentoFormatado()==''
                ...
                $objInfraException->lancarValidacao('Щ necessсrio informar ao menos um dos parтmetros da pesquisa.');
    	...
    	if ($arrDto==null){
    		$objInfraException->lancarValidacao('Nenhum documento encontrado com os parтmetros informados.');
      }
      ...
      foreach($arrDto as $objDocumentoDTOEncontrado) {
           ...
           * if ($objUnidadeDTO != null) {
              ...
              if (count($objProtocoloRN->pesquisarRN0967($objPesquisaProtocoloDTO)) == 0) {
                  $objInfraException->lancarValidacao('Unidade [' . $objUnidadeDTO->getStrSigla() . '] nуo possui acesso ao documento [' . $objDocumentoDTO->getStrProtocoloDocumentoFormatado() . '].');
              }
          }*/
          ...
          if ($objOperacaoServicoRN->contar($objOperacaoServicoDTO) == 0) {
              if ($objUnidadeDTO == null) {
                  $objInfraException->lancarValidacao('Nenhum serviчo configurado para consulta deste documento [' . $objDocumentoDTO->getStrProtocoloDocumentoFormatado() . '] pelo Serviчo [' . $objServicoDTO->getStrIdentificacao() . '].');
              } else {
                  $objInfraException->lancarValidacao('Nenhum serviчo configurado para consulta deste documento [' . $objDocumentoDTO->getStrProtocoloDocumentoFormatado() . '] na unidade [' . $objUnidadeDTO->getStrSigla() . '] pelo Serviчo [' . $objServicoDTO->getStrIdentificacao() . '].');
              }
          ...
          //verifica se o usuсrio jс tem acesso ao processo
      ...
      throw new InfraException('Erro no serviчo de consulta de documento.',$e);
    }
  }
    ...
    protected function listarProcedimentoControlado(WSEntradaListarProcedimentoDTO $objWSEntradaListarProcedimentoDTO){
        ...
        try{
            ...
            if ($arrDto==null){
                $objInfraException->lancarValidacao('Nenhum processo encontrado para os parтmetros informados.');
            }
            ...
            foreach($arrDto as $objProcedimentoDTO) {
                ...
                if ($objUnidadeDTO != null) {
                    ...
                    $objProtocoloRN = new ProtocoloRN();
                    if (count($objProtocoloRN->pesquisarRN0967($objPesquisaProtocoloDTO)) == 0) {
                        $objInfraException->lancarValidacao('Unidade [' . $objUnidadeDTO->getStrSigla() . '] nуo possui acesso ao processo [' . $objProcedimentoDTO->getStrProtocoloProcedimentoFormatado() . '].');
                    }
                }
                ...
                if ($objOperacaoServicoRN->contar($objOperacaoServicoDTO) == 0) {
                    if ($objUnidadeDTO == null) {
                        $objInfraException->lancarValidacao('Nenhum serviчo configurado para consulta deste processo [' . $objProcedimentoDTO->getStrProtocoloProcedimentoFormatado() . '] pelo Serviчo [' . $objServicoDTO->getStrIdentificacao() . '].');
                    } else {
                        $objInfraException->lancarValidacao('Nenhum serviчo configurado para consulta deste processo [' . $objProcedimentoDTO->getStrProtocoloProcedimentoFormatado() . '] na unidade [' . $objUnidadeDTO->getStrSigla() . '] pelo Serviчo [' . $objServicoDTO->getStrIdentificacao() . '].');
                    }
                }
            ...
            throw new InfraException('Erro no serviчo de consulta de procedimento.',$e);
        }
    }
    ...
    protected function listarAndamentosControlado(WSEntradaListarAndamentosDTO $objWSEntradaListarAndamentosDTO){
        try{
            ...
            if ($dto==null){
                $objInfraException->lancarValidacao('Processo '.$objProcedimentoDTO->getStrProtocoloProcedimentoFormatado().' nуo encontrado.');
            }
            ...
            throw new InfraException('Erro no serviчo de consulta de andamentos.',$e);
        }
  }
  ...
  private function prepararParticipantes($Interessado){
      ...
      //como nуo tem paginaчуo щ bom limitar
    ...
    private function prepararClassificacaoAssunto($ClassificacaoAssunto){
        ...
        if (count($arrAssuntos)==0){
            throw new InfraException('Nenhuma Classificaчуo de Assunto encontrado!'); }
  ...
  private function prepararUnidadeProcedimentoAberto($numIdUnidade, $numIdUsuario){
      ...
      if (count($arrObjPendenciaDTO)==0){
          throw new InfraException('Nenhum Procedimento com Pendъncia para a Unidade Informada!'); }
  ...
  private function adicionarCriteriosProcessoDocumento(OperacaoServicoDTO $objOperacaoServicoDTO, ProcedimentoDTO $objProcedimentoDTO, DocumentoDTO $objDocumentoDTO){
    ...
    //qualquer sщrie em qualquer tipo de procedimento
    ...
    //esta sщrie em qualquer tipo de procedimento
    $objOperacaoServicoDTO->adicionarCriterio(array('IdSerie','IdTipoProcedimento'),
        array(InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL),
    ...
    //qualquer sщrie neste tipo de procedimento
    $objOperacaoServicoDTO->adicionarCriterio(array('IdSerie','IdTipoProcedimento'),
        array(InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL),
    ...
    //esta sщrie neste tipo de procedimento
    $objOperacaoServicoDTO->adicionarCriterio(array('IdSerie','IdTipoProcedimento'),
        array(InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL),
  ...
  private function adicionarCriteriosUnidadeProcessoDocumento(OperacaoServicoDTO $objOperacaoServicoDTO, UnidadeDTO $objUnidadeDTO, ProcedimentoDTO $objProcedimentoDTO, DocumentoDTO $objDocumentoDTO){
    ...
    //qualquer sщrie em qualquer unidade em qualquer tipo de procedimento
    ...
    //esta sщrie em qualquer unidade em qualquer tipo de procedimento
    $objOperacaoServicoDTO->adicionarCriterio(array('IdSerie','IdUnidade','IdTipoProcedimento'),
        array(InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL),
    ...
    //qualquer sщrie nesta unidade em qualquer tipo de procedimento
    $objOperacaoServicoDTO->adicionarCriterio(array('IdSerie','IdUnidade','IdTipoProcedimento'),
        array(InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL),
    ...
    //qualquer sщrie em qualquer unidade neste tipo de procedimento
    $objOperacaoServicoDTO->adicionarCriterio(array('IdSerie','IdUnidade','IdTipoProcedimento'),
        array(InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL),
    ...
    //esta sщrie nesta unidade em qualquer tipo de procedimento
    $objOperacaoServicoDTO->adicionarCriterio(array('IdSerie','IdUnidade','IdTipoProcedimento'),
        array(InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL),
    ...
    //esta sщrie em qualquer unidade neste tipo de procedimento
    $objOperacaoServicoDTO->adicionarCriterio(array('IdSerie','IdUnidade','IdTipoProcedimento'),
        array(InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL),
    ...
    //qualquer sщrie nesta unidade neste tipo de procedimento
    $objOperacaoServicoDTO->adicionarCriterio(array('IdSerie','IdUnidade','IdTipoProcedimento'),
        array(InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL),
    ...
    //esta sщrie nesta unidade neste tipo de procedimento
    $objOperacaoServicoDTO->adicionarCriterio(array('IdSerie','IdUnidade','IdTipoProcedimento'),
        array(InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL,InfraDTO::$OPER_IGUAL),
